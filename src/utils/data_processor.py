...
def clean_dataset_basic(df):
    ...
    """
    logger.info("Running clean_dataset_basic")
    if df is None:
    ...
    logger.debug(f"Missing value threshold for column removal: {missing_threshold}")
    ...
    if "Age" in df_clean.columns:
        ...
        if age_missing > 0:
            ...
            logger.info(f"Replaced {age_missing} missing values in 'Age' with the median ({age_median:.1f})")
            st.info(f"Replaced {age_missing} missing values in 'Age' with the median ({age_median:.1f})")
    ...
    if "Embarked" in df_clean.columns:
        ...
        if embarked_missing > 0:
            ...
            logger.info(f"Replaced {embarked_missing} missing values in 'Embarked' with the mode ('{embarked_mode}')")
            st.info(f"Replaced {embarked_missing} missing values in 'Embarked' with the mode ('{embarked_mode}')")
...
def detect_outliers_iqr(series, lower_q=0.25, upper_q=0.75, multiplier=1.5):
    """
    Detect outliers using the IQR method from the notebook.
    """
...
def detect_outliers_summary(df):
    """
    Create an outlier summary for all numerical variables.
    Based on notebook section 4.1.1 analysis.
    """
    logger.info("Running detect_outliers_summary")
    if df is None:
        logger.warning("Empty input DataFrame")
        return None
...
def handle_outliers(df, method="clip", columns=None):
    """
    Handle outliers using notebook-based methods.
    """
    logger.info(f"Running handle_outliers (method: {method})")
    if df is None:
        logger.warning("Empty input DataFrame")
        return None
    ...
    for col in columns:
        if col in df_processed.columns:
            ...
            if len(outliers) > 0:
                logger.debug(f"Handling {len(outliers)} outliers in {col} using method {method}")
...
def create_basic_features(df):
    """
    Create base features used in later analyses.
    """
    logger.info("Running create_basic_features")
    if df is None:
        logger.warning("Empty input DataFrame")
        return None
    ...
    if "Age" in df_featured.columns:
        ...
        logger.debug("Created feature 'Age_Group'")
    ...
    if "Fare" in df_featured.columns:
        ...
        logger.debug("Created feature 'Fare_Category'")
...
def validate_data_quality(df):
    """
    Validate the overall quality of the dataset.
    """
    logger.info("Running validate_data_quality")
    if df is None:
        logger.warning("Empty input DataFrame")
        return None
