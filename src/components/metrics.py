...
from src.config import VALUE_MAPPINGS, format_percentage
...
def create_overview_metrics(df):
    ...
    Based on the notebook survival analysis in section 4.2.2.
    """
    if df is None:
        return

    # Base survival calculations
    total_passengers = len(df)
    survived = df["Survived"].sum()
    died = total_passengers - survived
    survival_rate = (survived / total_passengers) * 100

    # Four-column layout for the main metrics
...
def create_class_metrics(df):
    ...
    class_survival = df.groupby("Pclass").agg({"Survived": ["sum", "count", "mean"]}).round(3)
...
def create_gender_metrics(df):
    ...
    gender_survival = df.groupby("Sex").agg({"Survived": ["sum", "count", "mean"]}).round(3)
...
def create_statistical_metrics(df):
    """
    Show the main descriptive statistics.
